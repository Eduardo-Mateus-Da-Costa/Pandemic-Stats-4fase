...
declare 
	codcom numeric := 0;
begin
	select pc.codcom_codcom into codcom from paciente_comorbidade pc
	where pc.codpac_codpac = new.codpac_codpac;
    if (new.codcom_codcom = codcom) then
...
select id, case when mod(id, 2)=0 then 'S' else 'N' end, (30.5+id), case when mod(id, 2)=0 then 'BEM' else 'ISOLAMENTO' end, id
...
insert into monitoramento_paciente(codmon, intsin, codpac_codpac, codsin_codsin) 
select nextval('seq_monitoramento_paciente'), case when mod(id, 2)=0 then 'C' else 'S' end, id, id-11
from generate_series(12, 22) as t(id);
...
from generate_series(1, 13) as t(id);

insert into paciente_comorbidade(codpaccom, codpac_codpac, codcom_codcom) 
select nextval('seq_paciente_comorbidade'), id, id-13
from generate_series(14, 22) as t(id);
