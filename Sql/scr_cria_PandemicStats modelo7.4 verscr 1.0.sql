...
create table medico (
codmed numeric(10, 0) not null constraint medico_pk primary key,
nommed varchar(100) not null,
crmmed varchar(30) not null unique,
cpfmed numeric(11, 0) not null unique
);


comment on table medico is 'Cadastro de médico';
...
comment on column medico.cpfmed is 'CPF do médico';


create table vacina (
codvac numeric(10, 0) not null constraint vacina_pk primary key,
datvac date not null default current_timestamp,
dosvac char(1) not null constraint vac_dosvac_ck check (dosvac in ('1','2')),
fabvac varchar(30),
codpac numeric(10,0) not null constraint vac_codpac_fk references paciente (codpac),
codmed numeric (10,0) not null constraint vac_codmed_fk references medico (codmed)
);
...
tipusu char(1) not null constraint usu_tipusu_ck check(tipusu in('a','c','p','m')),
...
codmed numeric (10, 0) constraint usu_codmed_fk references medico (codmed)
);
